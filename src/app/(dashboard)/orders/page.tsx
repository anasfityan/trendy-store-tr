...
async function openInvoice(order: Order) {
  ...
  let storeInstagram = "";
  let storeLogo = "";
  let storeCity = "Istanbul, Türkiye";
  try {
    const res = await fetch("/api/settings");
    if (res.ok) {
      const s = await res.json();
      if (s.storeName) storeName = s.storeName.toUpperCase();
      if (s.phone) storePhone = s.phone;
          ...
          <div className="space-y-2 pb-32">
            {orders.map((order, idx) => {
              ...
              return (
                <div
                  key={order.id}
                  className={`relative bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-3 animate-fade-in-up ${statusDropId === order.id ? "z-[100]" : "z-0"}`}
                  style={{ animationDelay: `${idx * 30}ms` }}
                >
                  {/* Image + info */}
